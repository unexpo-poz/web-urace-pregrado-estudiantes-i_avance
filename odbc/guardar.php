...
$mSQL  = "select distinct EXP_E from N_ESTU where acta='$nacta' and lapso='$nlapso' and c_asigna='$nc_asigna'";
$conex->ExecSQL($mSQL,__LINE__,true);
$result = $conex->result;
if($result[0][0]!=NULL)
{
...
//require_once("inc/odbcss_c.php");
global $conex;
//$conex = new ODBC_Conn($basededatos,$usuariodb,$clavedb,TRUE,'notas.log');
...
if($codigos==987789){ ///////mostrando las calificaciones de una materia con fajax
$acta=$_POST['valor'];
$exp_e=$_POST['exp_e'];
$np=$_POST['np'];
global $conex;
global $lapsoProceso;
$mSQL  = "select c_asigna from tblaca004 where acta='$acta' and lapso='$lapsoProceso' ";
$conex->ExecSQL($mSQL,__LINE__,true);
$c_asigna = $conex->result;
				$activasta=comprov($acta,$lapsoProceso,$c_asigna[0][0]);
				$datos=sacanotasindi($acta,$lapsoProceso,$c_asigna[0][0],$exp_e);
				$fechas=sacafecha($acta,$lapsoProceso,$c_asigna[0][0]);
				$porc= array();
				$tema= array();
				$resultado=con_ex_temas_car($acta,$lapsoProceso,$c_asigna[0][0]);
				if($resultado!=NULL)
					{	
					$cantee=$resultado[0][0];
					for($i=1;$i<=35;$i++) $tema[$i]=$resultado[0][$i];
...
$mSQL  = "select a.seccion,c.asignatura,d.apellido||', '||nombre  from dace006 a,tblaca004 b,tblaca008 c,tblaca007 d  where b.acta='$acta' and b.c_asigna=b.c_asigna and b.lapso='$lapsoProceso' and a.lapso='$lapsoProceso' and a.exp_e='$exp_e' and c.c_asigna=b.c_asigna and a.acta=b.acta and a.seccion=b.seccion and b.ci=d.ci";
$conex->ExecSQL($mSQL,__LINE__,true);
$re = $conex->result;
echo '<table class="datotabla"><tr><td width="100%">';
	...
	echo			'</tr>
					...
					for($i=1;$i<=$cantee;$i++) echo'<td align="center" >'.$porc[$i].'</td>';
					echo'<td align="center" width="80" ><strong>100%</strong></td><td align="center" width="80" ><strong>9</strong></td></tr>';
echo            '
